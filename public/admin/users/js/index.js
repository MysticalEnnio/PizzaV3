...
let setPriorityButton = document.getElementById("setPriorityButton");
...
changeUserNameButton.addEventListener("click", async () => {
    ...
    if (data)
        ...
        Swal.fire(
            "Changed!",
            `The users name been changed to ${name}.`,
            "success"
        );
});

setPriorityButton.addEventListener("click", async () => {
    //1-6
    ...
    const data = await saveUserData(userEditUser, {
        user_metadata: { priority: priority },
    });
    ...
    if (data)
        ...
        Swal.fire(
            "Changed!",
            `The users priority been changed to ${priority}.`,
            "success"
        );
});
